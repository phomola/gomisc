...
func getCopier2(dst, src reflect.Type) (func(unsafe.Pointer, unsafe.Pointer) error, error) {
	pair := typePair{dst, src}
	if f, ok := copiers2.Load(pair); ok {
		return f.(func(unsafe.Pointer, unsafe.Pointer) error), nil
	}
	if dst.Kind() != reflect.Struct || src.Kind() != reflect.Struct {
		return nil, ErrInvalidType
		...
		case fs.Type.Kind() == reflect.Struct && fd.Type.Kind() == reflect.Struct:
			if fs.Type == fd.Type {
				fieldCopiers = append(fieldCopiers, func(dst, src unsafe.Pointer) error {
					dst = unsafe.Add(dst, do)
					src = unsafe.Add(src, so)
					d := reflect.NewAt(fd.Type, dst)
					s := reflect.NewAt(fs.Type, src)
					d.Elem().Set(s.Elem())
					return nil
				})
			} else {
	...
	copiers2.Store(pair, f)
	return f, nil
}

// Copy2 ...
func Copy2[D, S any](dst *D, src *S) error {
	f, err := getCopier2(reflect.TypeFor[D](), reflect.TypeFor[S]())
	if err != nil {
		return err
	}
	return f(unsafe.Pointer(dst), unsafe.Pointer(src))
...
// Copied2 ...
func Copied2[D, S any](src *S) (*D, error) {
	var dst D
	if err := Copy2(&dst, src); err != nil {
		return nil, err
	}
	return &dst, nil
}
